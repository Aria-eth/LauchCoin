...
const blockchains = [
  ...
  'Arbitrum', 'Optimism', 'Base', 'Other'
];

const languages = [
  'Solidity', 'Vyper', 'Rust', 'Move', 'Cairo', 'Other'
];
        ...
        <div className="container mx-auto px-4 relative z-10 pt-24">
          ...
          >
            ...
            <p className="text-xl text-gray-300">
              Get a comprehensive security audit from industry-leading experts. Fill out the form below and we'll contact you within 24 hours.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24 relative">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
                  ...
                  <React.Fragment key={step.number}>
                    ...
                    {index < steps.length - 1 && (
                      <div className={`flex-1 h-0.5 mx-4 transition-all ${
                        currentStep > step.number ? 'bg-primary-400' : 'bg-gray-700'
                      }`} />
                    )}
                  </React.Fragment>
                ))}
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                  ...
                  >
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <select {...register('blockchain')} className="input-field">
                        ...
                        {errors.blockchain && (
                          <p className="mt-1 text-sm text-red-400">{errors.blockchain.message}</p>
                        )}
                      </div>
                ...
                {currentStep === 2 && (
                  <motion.div
                    ...
                    className="space-y-6"
                  >
                    <h2 className="text-2xl font-semibold text-white mb-6">Audit Requirements</h2>
                    
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-2">
                        Audit Type *
                      </label>
                      <select {...register('auditType')} className="input-field">
                      ...
                      {errors.auditType && (
                        <p className="mt-1 text-sm text-red-400">{errors.auditType.message}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        {[
                          { value: 'standard', label: 'Standard', time: '2-3 weeks', icon: Clock },
                          { value: 'express', label: 'Express', time: '1 week', icon: Zap },
                          { value: 'emergency', label: 'Emergency', time: '48 hours', icon: AlertCircle }
                        ].map(option => (
                          <label
                            key={option.value}
                          ...
                          >
                            ...
                            <div className="text-center">
                              <option.icon className={`w-8 h-8 mx-auto mb-2 ${
                                urgency === option.value ? 'text-primary-400' : 'text-gray-500'
                              }`} />
                              <p className="font-medium text-white">{option.label}</p>
                              <p className="text-xs text-gray-400 mt-1">{option.time}</p>
                            </div>
                          </label>
                        ))}
                      </div>
                    ...
                    <div className="bg-primary-500/10 border border-primary-500/30 rounded-lg p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="text-sm text-gray-300">
                          <p className="font-medium text-primary-400 mb-1">What happens next?</p>
                          <ul className="space-y-1">
                            ...
                            <li className="flex items-start gap-2">
                              <CheckCircle className="w-4 h-4 text-green-400 flex-shrink-0 mt-0.5" />
                              <span>Our team will contact you to discuss requirements</span>
                            </li>
                            <li className="flex items-start gap-2">
                              <CheckCircle className="w-4 h-4 text-green-400 flex-shrink-0 mt-0.5" />
                              <span>You'll receive a detailed audit proposal and timeline</span>
                            </li>
                          </ul>
                        </div>
                      </div>
            ...
            >
              {[
                { icon: Shield, title: 'Secure Process', text: 'All information is encrypted and confidential' },
                { icon: Clock, title: 'Fast Response', text: 'We respond within 24 hours' },
                { icon: DollarSign, title: 'Transparent Pricing', text: 'Clear pricing with no hidden fees' }
              ].map((feature, index) => (
